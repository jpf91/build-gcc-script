...
bool handleArguments(string[] args)
{
    ...
    void setBuildVariable(string opt, string val)
    {
        ...
        failEnforcec(cast(bool) parts, "Invalid format for build variable: ",
            val, " usage: --variable=key:val");
        cmdVariables[parts[0]] = parts[2];
    }

    auto helpInformation = getopt(args, "build", "Build machine triplet",
        &buildTriplet, "host", "Overwrite host machine triplet",
        &cmdOverwrites.host, "target", "Overwrite target machine triplet",
        &cmdOverwrites.target, "mirrors", "File containing mirror urls", (string opt,
        string val) { mirrorFile = Path(val); }, "cache",
        "Path to cache folder", (string opt, string val) { cacheDir = Path(val); },
        "verify-cached-sources", "Verify cached source files",
        &verifyCachedSources, "force-download",
        "Download even if sources exist", &forceDownload, "force-extract",
        "Extract even if sources already extracted", &forceExtract,
        "num-cpus", "Number of CPUs to use", &numCPUs, "skip-target-strip",
        "Do not strip target libraries", &skipStripLibraries,
        "skip-host-strip", "Do not strip host executables",
        &skipStripBinaries, "keep-build-files",
        "Do not delete objects after build", &keepBuildFiles, "variable",
        "Set build variable (--variable=key:val)", &setBuildVariable,
        ...
        "target-strip", "Command to strip binaries for target", &targetStripCMD,
    ...
    if (!helpInformation.helpWanted)
    {
        ...
        case 3:
            ...
            mode = BuildMode.all;
            failEnforcec(buildConfig.exists, "Invalid path for build settings file: ",
                buildConfig);
    ...
    if (helpInformation.helpWanted || printHelp)
    {
        ...
        foreach (it; helpInformation.options)
        {
            writefln("    %-*s %-*s%s%s", lenShort, it.optShort, lenLong,
                it.optLong, it.required ? "  Required: " : "  ", it.help);
        }
